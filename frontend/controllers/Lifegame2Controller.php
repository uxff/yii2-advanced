...
class Lifegame2Controller extends \yii\web\Controller
{
    private $_width = 20;
    private $_height = 20;
    private $_deep = 32;
    ...
    const MC_KEY_PRE        = 'lg2_map_0';
    const MC_KEY_VER_PRE    = 'lg2_map_version';
    const DEFAULT_PX = 2;
    ...
    public function actionGdmaphtml() {
        echo '<img src="index.php?r=lifegame2/gdmap"/>';
    }
    ...
    public function actionShowtdmap() {
        ...
        for ($i=0; $i<$this->_width * $this->_height; ++$i)
        {
            ...
            $y = (int)($i/$this->_width);
            $x = (int)($i%$this->_width);
            $mapDot = isset($map[$y.'_'.$x]) ? $map[$y.'_'.$x] : 0;
            $neighberStr = '';
            // 外围第一圈 显示
    ...
    public function actionMakelife() {
        ...
        $this->redirect(['lifegame2/showmap', 'ver'=>$newver, 'px'=>$px]);
    }
    // 生存规则
    /*
    ...
    protected function makeLife($map) {
        ...
        foreach ($map as $pos=>$mapDot) {
            ...
            if (!$mapDot) {
                ...
                // 尝试周围的点
                $newMap = array_merge($newMap, $this->tryCreateLife($x, $y, $map));
            }
            //$this->_lifeCount += $newMap[$i];
        }
        $this->_lifeCount = count($newMap);
        return $newMap;
    }
    /*
    ###
    ...
    */
    ...
    protected function calcNeighber($x, $y, $map) {
        ...
        foreach ($dotPos as $dot) {
            ...
            $pos = $dot[1].'_'.$dot[0];
            $val = isset($map[$pos]) ? 1 : 0;
            $sum += $val;
        }
        return $sum;
    }
    // 
    protected function tryCreateLife($x, $y, $map) {
        // 找到某点周边的空白点
        $arrPos = [];
        $dotPos = $this->getNeighberPos($x, $y);
        foreach ($dotPos as $dot) {
    ...
    public function actionClearmap($ver) {
        ...
        $this->redirect(['lifegame2/showmap', 'ver'=>$ver]);
    }
    public function actionMakenewmap() {
        ...
        $this->redirect(['lifegame2/showmap', 'ver'=>$newver]);
    }
    protected function makeMap($width = 0) {
        ...
        */
        ...
        $arr[($mHeight-1).'_'.($mWidth-0)] = 1;$arr[($mHeight-1).'_'.($mWidth+1)] = 1;
        $arr[($mHeight+0).'_'.($mWidth-1)] = 1;$arr[($mHeight+0).'_'.($mWidth-0)] = 1;
        $arr[($mHeight+1).'_'.($mWidth-0)] = 1;
        return $arr;
    }
    /*
    找到周围如下坐标 距离为2
